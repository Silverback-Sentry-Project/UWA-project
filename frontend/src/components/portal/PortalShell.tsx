...
import {
  LayoutDashboard, FileWarning, Flame, UserCheck,
  ScrollText, Settings, Bell, ChevronDown, LogOut,
  ChevronRight, HelpCircle, CheckCircle2, Clock, AlertTriangle, Play, Zap,
} from "lucide-react";
...
const NAV_SECTIONS: { title: string; items: NavItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: "/portal/conflicts", label: "Conflict Alerts", icon: ScrollText, hint: "Emergency response" },
    ],
  },
];
...
export function PortalShell({ children, title, subtitle, actions, helpText }: { children: ReactNode; title: string; subtitle?: string; actions?: ReactNode; helpText?: string }) {
  ...
  interface Park { park_id: string; park_name: string }
    ...
    <div className="portal h-screen w-full flex overflow-hidden">
      ...
      <aside className="w-64 shrink-0 bg-[#1A2F1A] text-white flex flex-col h-full border-r border-white/5">
        <div className="px-6 py-6 border-b border-white/5 flex items-center gap-3">
          ...
          <div>
            <div className="font-bold portal-display text-[16px] leading-tight tracking-tight">WildWatch</div>
            <div className="text-[10px] uppercase tracking-[0.1em] text-white/50 font-semibold">HQ Control</div>
          </div>
        </div>
        <nav className="flex-1 px-3 py-6 space-y-6 overflow-auto custom-scrollbar">
          {NAV_SECTIONS.map((sec) => (
            <div key={sec.title}>
              <div className="px-3 mb-2 text-[10px] uppercase tracking-[0.2em] text-white/30 font-bold">{sec.title}</div>
                ...
                {sec.items.map((n) => {
                  ...
                  return (
                    <Link key={n.to} to={n.to} title={n.hint}
                      ...
                      <n.icon size={18} className={`transition-colors ${active ? "text-[#C4A456]" : "text-white/40 group-hover:text-white/60"}`} />
                      <span className="flex-1">{n.label}</span>
                      {active && <div className="h-1.5 w-1.5 rounded-full bg-[#C4A456] shadow-[0_0_8px_rgba(196,164,86,0.6)]" />}
                    </Link>
                  );
                })}
              </div>
        ...
        <div className="border-t border-white/5 p-4 space-y-2 bg-black/10">
          <Link to="/portal/settings" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium text-white/50 hover:bg-white/5 hover:text-white transition-all">
            <Settings size={18} className="text-white/30" />
            <span>Settings</span>
          </Link>

          <button onClick={handleSignOut} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium text-[#FF8A80] hover:bg-white/5 transition-all">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full min-h-0 bg-[#F8F9FA]">
        <header className="h-16 shrink-0 bg-white border-b border-neutral-200 flex items-center justify-between px-8 shadow-sm z-10">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-neutral-100 rounded-lg border border-neutral-200">
               <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Scoping</span>
               <select
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              >
                ...
                {unreadCount > 0 && <span className="absolute top-2 right-2.5 h-2 w-2 rounded-full bg-[#D32F2F] border-2 border-white shadow-sm" />}
              </button>
              {notifOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-3 w-80 bg-white border border-neutral-200 rounded-2xl shadow-2xl z-50 max-h-[480px] overflow-hidden flex flex-col text-[#1A1A1A] animate-in fade-in zoom-in-95 duration-200">
                    <div className="px-4 py-3 border-b border-neutral-100 flex items-center justify-between bg-neutral-50/50">
                      ...
                      {unreadCount > 0 && <span className="text-[10px] font-bold bg-[#D32F2F] text-white px-2 py-0.5 rounded-full">{unreadCount}</span>}
                    </div>
                    <div className="flex-1 overflow-auto custom-scrollbar">
                      {(notifications ?? []).length === 0 && (
                        <div className="px-4 py-12 text-center text-[12px] text-neutral-400 font-medium">All caught up.</div>
                      )}
                      {(notifications ?? []).map((n) => (
                        <button
                          key={n.notification_id}
                          onClick={() => { markRead(n.notification_id); setNotifOpen(false); }}
                        ...
                        >
                          <div className="flex items-start gap-3">
                            {!n.is_read && <div className="mt-1.5 h-1.5 w-1.5 rounded-full bg-blue-500 shrink-0" />}
                            <div className="flex-1 min-w-0">
                              <div className="text-[12px] font-bold text-neutral-800 leading-tight">{n.title}</div>
                              <div className="text-[11px] text-neutral-500 mt-1 line-clamp-2 leading-relaxed">{n.message}</div>
                              <div className="text-[10px] text-neutral-400 mt-2 font-bold uppercase tracking-wider">{new Date(n.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                            </div>
                          </div>
                        </button>
                      ))}
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-neutral-100">
              <div className="h-10 w-10 rounded-full bg-gradient-to-br from-[#1A2F1A] to-[#2E4D2E] text-white flex items-center justify-center text-[13px] font-bold shadow-md border-2 border-white">{initials}</div>
            </div>
          </div>
        </header>
        ...
        <div className="px-8 pt-8 pb-6 shrink-0">
          <div className="flex items-center gap-2 text-[11px] text-neutral-400 font-bold uppercase tracking-[0.1em] mb-2">
            <Link to="/portal/dashboard" className="hover:text-[#1A2F1A] transition-colors">HQ</Link>
            {section && <><ChevronRight size={10} className="text-neutral-300" /><span>{section.title}</span></>}
          </div>

          <div className="flex items-center justify-between gap-4">
            <div className="min-w-0">
              <h1 className="text-[28px] font-black portal-display text-neutral-900 tracking-tight leading-tight">{title}</h1>
              {subtitle && <p className="text-[14px] text-neutral-500 font-medium mt-1">{subtitle}</p>}
            </div>
...
export function StatCard({ label, value, delta, tone = "olive", icon: Icon, to, hint }: { label: string; value: string | number; delta?: string; tone?: "olive" | "gold" | "danger" | "info"; icon?: any; to?: string; hint?: string }) {
  ...
  const inner = (
    <div className="flex flex-col h-full justify-between">
      <div className="flex items-center justify-between">
        <span className="text-[11px] font-black uppercase tracking-[0.15em] text-neutral-400">{label}</span>
        {Icon && (
          <div className="h-10 w-10 rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:scale-110" style={{ background: t.bg, color: t.main }}>
            <Icon size={18} />
          </div>
        )}
      </div>
      <div className="mt-4">
        <div className="flex items-baseline gap-2">
          <span className="text-4xl font-black portal-display tracking-tight text-neutral-900">{value}</span>
          {delta && <span className="text-[12px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-md">{delta}</span>}
        </div>
        {hint && <div className="mt-2 text-[12px] text-neutral-400 font-medium leading-tight">{hint}</div>}
      </div>
      {to && (
  ...
  if (to) {
    return <Link to={to} className="portal-card p-6 block hover:shadow-xl hover:border-neutral-300 transition-all duration-300 group bg-white">{inner}</Link>;
  }
...
export function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-lg capitalize border border-black/5 shadow-sm transition-all"
