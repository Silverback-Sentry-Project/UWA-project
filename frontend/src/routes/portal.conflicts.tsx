        ...
        <div className="grid grid-cols-2 gap-6">
          {alerts.length === 0 && (
             <div className="col-span-2 portal-card p-12 text-center bg-white shadow-sm border-neutral-100">
                ...
                <p className="text-[13px] text-neutral-400 font-medium">No emergency signals detected in this sector.</p>
             </div>
          )}
          {alerts.map((a) => (
            <div key={a.sos_id} className="portal-card p-6 flex gap-6 bg-white shadow-sm hover:shadow-md transition-shadow border-neutral-100 group">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <div className="font-black text-[11px] uppercase tracking-[0.15em] text-neutral-400">SIGNAL SOS-{a.sos_id}</div>
                  <StatusBadge status={a.status} />
                </div>
                <h4 className="portal-display text-lg font-black text-neutral-900 mt-1">{a.emergency_type}</h4>
                <p className="text-[13px] text-neutral-500 mt-2 line-clamp-2 leading-relaxed font-medium">{a.description ?? "No telemetry provided."}</p>
                <div className="mt-4 pt-4 border-t border-neutral-50 flex items-center justify-between text-[11px] font-bold">
                  <span className="text-neutral-400 uppercase tracking-widest">{a.park?.park_name ?? "Sector Unknown"} · {fmtDate(a.created_at)}</span>
                  {a.reporter && <span className="text-[#1A2F1A] bg-emerald-50 px-2 py-0.5 rounded-md">via {a.reporter.first_name}</span>}
                </div>
                {a.status !== "Resolved" && (
                  <div className="mt-6 flex gap-3">
                    {a.status === "Pending" && (
                      <button className="portal-btn flex-1 justify-center py-2.5 h-10 shadow-sm" disabled={busyId === a.sos_id} onClick={() => setStatus(a.sos_id, "Responding")}>Deploy Response</button>
                    )}
                    <button className="portal-btn portal-btn-ghost flex-1 justify-center py-2.5 h-10 border-neutral-200" disabled={busyId === a.sos_id} onClick={() => setStatus(a.sos_id, "Resolved")}>Signal Clear</button>
                  </div>
                )}
              </div>
            </div>
