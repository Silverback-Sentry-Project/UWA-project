...
function GameparkPersonnel() {
  ...
  const { data: people, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiFetch<PersonRow[]>(
        `/gamepark/personnel${selectedParkId ? `?park_id=${selectedParkId}` : ""}`,
      ),
  });
  ...
  const invitableRoles = (allRoles ?? []).filter((r) =>
    INVITABLE_ROLES.some((i) => i.role_name === r.role_name),
  );
  ...
  async function sendInvite() {
    ...
    setSending(true);
    try {
      const res = await apiFetch<{ mail_sent: boolean; message: string }>(
        "/gamepark/personnel/invite",
        {
          method: "POST",
          body: JSON.stringify({
  ...
  return (
    <PortalShell
      title="Settings"
      subtitle="Field staff and access management for this park."
      actions={
        ...
        <div className="portal-card overflow-hidden">
          <table className="portal-table">
            ...
            <tbody>
              {(people ?? []).length === 0 && (
                <tr>
                  <td colSpan={4} className="text-center text-[var(--p-ink-soft)] py-4">
                    No personnel invited yet.
                  </td>
                </tr>
              )}
              {(people ?? []).map((p) => (
                <tr key={p.user_id}>
                  ...
                  <td>
                    {(p.roles ?? []).map((r) => (
                      <span key={r.role_name} className="portal-chip mr-1">
                        {r.role_name}
                      </span>
                    ))}
                  </td>
                  <td>
                    <span className="portal-chip">{p.account_status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="bg-white rounded-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-[var(--p-olive-line)]">
              ...
              <p className="text-[12px] text-[var(--p-ink-soft)] mt-0.5 flex items-center gap-1.5">
                <ShieldCheck size={12} /> They'll be registered as working for this park, and
