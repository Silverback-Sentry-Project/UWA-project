...
const ITEMS = [
  ...
  },
  {
    ...
    time: "Today",
    tone: "bg-accent/20 text-foreground",
    unread: false,
  },
  {
    icon: Megaphone,
        ...
        <div className="px-5 pb-6 space-y-2">
          {ITEMS.map((n, i) => {
            ...
            return (
              <div
                key={i}
                className={`bg-card rounded-2xl p-3 shadow-card flex items-start gap-3 ${n.unread ? "ring-1 ring-primary/20" : ""}`}
              >
                <div className={`h-10 w-10 rounded-xl grid place-items-center ${n.tone}`}>
                  <Icon size={18} />
                </div>
