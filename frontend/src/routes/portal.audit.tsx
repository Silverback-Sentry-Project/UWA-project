...
function Audit() {
  ...
  return (
    <PortalShell
      title="Audit Log"
      subtitle="Record of incident status changes across the portal."
    ...
    >
      {isLoading && (
        <div className="portal-card p-6 text-sm text-[var(--p-ink-soft)]">Loading audit log…</div>
      )}
      {isError && (
        <div className="portal-card p-6 text-sm text-[var(--p-danger)]">
          Couldn't load audit log: {error instanceof Error ? error.message : "unknown error"}
        </div>
      )}
      ...
      {data && (
        <div className="portal-card overflow-hidden">
          <div className="px-4 py-3 border-b border-[var(--p-olive-line)] bg-[var(--p-olive-soft)] flex items-center gap-2 text-[11px] text-[var(--p-olive-deep)]">
            <ShieldCheck size={13} />{" "}
            <span className="font-semibold">{entries.length} recorded status changes</span>
          </div>
          <table className="portal-table">
            ...
            <tbody>
              {entries.length === 0 && (
                <tr>
                  <td colSpan={6} className="text-center text-[var(--p-ink-soft)] py-4">
              ...
              {entries.map((a) => (
                <tr key={a.status_history_id}>
                  ...
                  <td className="font-semibold">
                    {a.updatedBy ? `${a.updatedBy.first_name} ${a.updatedBy.last_name}` : "—"}
                  </td>
