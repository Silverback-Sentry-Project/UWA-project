      ...
      <div className="min-h-full flex flex-col bg-background">
        ...
        <div className="px-5 pb-6 space-y-4">
          <div className="bg-card rounded-2xl p-5 shadow-card text-center">
            <div className="h-20 w-20 rounded-full mx-auto gradient-forest grid place-items-center text-white text-2xl font-bold">
              J
            </div>
            <div className="mt-3 text-lg font-bold">Ranger Joseph Okello</div>
            <div className="text-xs text-muted-foreground flex items-center justify-center gap-1">
              <MapPin size={12} />
              Murchison Falls · Sector 3
            </div>
          ...
          <Section title="Field">
            <Row
              ...
              right={<span className="text-xs text-muted-foreground">Sector 3, 4</span>}
            />
            <Row
              icon={Clock}
              label="Shift preferences"
              right={<span className="text-xs text-muted-foreground">06:00 – 18:00</span>}
            />
...
function Section({ title, children }: { title: string; children: any }) {
  return (
    <div>
      ...
      <div className="bg-card rounded-2xl shadow-card p-1">{children}</div>
    </div>
  );
}
