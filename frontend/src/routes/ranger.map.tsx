  ...
  return (
    <PhoneFrame>
      <div className="min-h-full flex flex-col bg-background">
        ...
        <div
          className="relative flex-1 mx-3 rounded-3xl overflow-hidden shadow-card"
          style={{ minHeight: 480 }}
        >
          {/* Map surface */}
          <div
          ...
          >
            ...
            >
              <path
                d="M 0 200 Q 100 150 200 220 T 400 180 L 400 300 Q 300 350 200 320 T 0 360 Z"
                fill="oklch(0.35 0.1 220 / 0.5)"
              />
              <path
                d="M 50 0 Q 80 200 120 400 T 180 600"
                stroke="oklch(0.85 0.04 95)"
                strokeWidth="2"
                strokeDasharray="6 6"
                fill="none"
              />
              <path
                d="M 280 0 Q 260 180 300 380 T 380 600"
                stroke="oklch(0.85 0.04 95)"
                strokeWidth="2"
                strokeDasharray="6 6"
                fill="none"
          ...
          <div className="absolute top-3 left-3 right-3 flex items-start justify-between">
            <div className="bg-card/95 backdrop-blur rounded-2xl p-1 flex shadow-card">
              {(["3D", "Sat"] as const).map((v) => (
                <button
                  key={v}
                  onClick={() => setView(v)}
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold ${view === v ? "bg-primary text-primary-foreground" : "text-foreground"}`}
                >
                  {v}
                </button>
              ))}
            ...
            <div className="flex flex-col gap-2">
              <button className="h-10 w-10 rounded-2xl bg-card/95 backdrop-blur shadow-card grid place-items-center">
                <Layers size={16} />
              </button>
              <button className="h-10 w-10 rounded-2xl bg-card/95 backdrop-blur shadow-card grid place-items-center">
                <Plus size={16} />
          ...
          <div className="absolute left-3 right-3 bottom-3 bg-card/95 backdrop-blur rounded-2xl shadow-card p-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Pill tone="info">
                <Users size={10} /> 4 rangers nearby
              </Pill>
            </div>
        ...
        <div className="px-5 pt-4 pb-2">
          <div className="bg-warning/15 rounded-2xl p-3 text-xs flex items-start gap-2">
            ...
            <div>
              <span className="font-bold">Ranger Joseph</span> moved to Sector 4 — avoid overlap.
              Coordinate via map chat.
            </div>
          </div>
        </div>
        <RangerTabBar />
...
function Marker({
  ...
  return (
    <div className="absolute -translate-x-1/2 -translate-y-full" style={{ left: x, top: y }}>
      <div
        className={`h-7 w-7 rounded-full ${color} border-2 border-white shadow-lg grid place-items-center text-white text-[10px] font-bold`}
      >
        {icon ? "R" : "!"}
      </div>
      <div className="absolute top-7 left-1/2 -translate-x-1/2 mt-1 bg-card/95 backdrop-blur px-2 py-0.5 rounded-md text-[9px] font-bold whitespace-nowrap shadow">
        {label}
      </div>
    </div>
  );
