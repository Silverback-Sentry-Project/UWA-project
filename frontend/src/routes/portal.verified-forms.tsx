...
function VerifiedForms() {
  ...
  return (
    <PortalShell title="Verified Forms" subtitle="Evidence forms verified by gameparks and forwarded here for further action.">
        ...
        <div className="col-span-5 space-y-2">
          {(submissions ?? []).map((s) => (
            ...
            >
              ...
              <div className="mt-1 flex items-center gap-1.5 text-[11px] text-[var(--p-ink-soft)]"><User size={11} /> {s.submitted_by_name ?? "Anonymous"}</div>
              <div className="mt-1 text-[10px] text-[var(--p-ink-soft)]">Forwarded {fmtDate(s.forwarded_at)}</div>
            </button>
          ))}
        </div>
        <div className="col-span-7">
          {!open && <div className="portal-card p-8 text-center text-[13px] text-[var(--p-ink-soft)]">Select a forwarded form to view details.</div>}
          {open && (
            <div className="portal-card p-4">
              <h3 className="portal-display text-sm font-bold">{open.form.title}</h3>
              <div className="text-[11px] text-[var(--p-ink-soft)] mt-1">
                {open.park.park_name} · Verified by {open.verifier?.full_name ?? "—"} · Forwarded {fmtDate(open.forwarded_at)}
              </div>
                ...
                {open.answers.map((a) => (
                  <div key={a.answer_id} className="border border-[var(--p-olive-line)] rounded-md p-2.5">
                    ...
                    {a.field.field_type === "image" ? (
                      a.image_path ? <img src={a.image_path} alt={a.field.label} className="mt-1 max-h-40 rounded-md" /> : <div className="text-[12px] text-[var(--p-ink-soft)] mt-0.5">No photo attached.</div>
                    ) : (
