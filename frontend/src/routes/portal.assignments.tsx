...
function Assignments() {
  ...
  async function assign(incidentId: number) {
    ...
    setErr(null);
    try {
      await apiFetch(`/gamepark/incidents/${incidentId}/assign`, {
        method: "POST",
        body: JSON.stringify({ ranger_id: Number(rangerChoice) }),
      });
  ...
  return (
    <PortalShell
      ...
      subtitle={`Strategic personnel allocation for field response teams.`}
    ...
    >
      <div className="grid grid-cols-12 gap-8">
        <div className="col-span-4 space-y-4">
          ...
          <div className="portal-card bg-white shadow-sm border-neutral-100 h-[700px] flex flex-col">
            ...
            <div className="flex-1 overflow-auto p-3 space-y-3 custom-scrollbar">
              {loadingIncidents && (
                <div className="text-[12px] text-neutral-400 p-2 italic animate-pulse">
                  Scanning field frequencies…
                </div>
              )}
              {!loadingIncidents && unassigned.length === 0 && (
                <div className="text-[12px] text-neutral-300 p-2 text-center py-12 italic">
                  Queue clear. All sectors quiet.
                </div>
              )}
              {unassigned.map((i) => (
                <div
                  key={i.incident_id}
                  className={`p-4 rounded-2xl border transition-all duration-300 ${assigning === i.incident_id ? "border-[#1A2F1A] bg-emerald-50/20 shadow-md" : "border-neutral-100 bg-white hover:border-neutral-200 shadow-sm"}`}
                >
                  <div className="flex items-center justify-between">
                    <div className="font-mono text-[10px] font-black text-neutral-300">
                      WW-{i.incident_id}
                    </div>
                    <StatusBadge status={i.status} />
                  </div>
                  <div className="text-[13px] font-black text-neutral-800 mt-2">
                    {i.incident_type}
                  </div>
                  <div className="text-[11px] text-neutral-500 line-clamp-2 mt-1 leading-snug font-medium">
                    {i.description}
                  </div>
                  ...
                  {assigning === i.incident_id ? (
                    <div className="mt-4 pt-4 border-t border-neutral-100 space-y-2">
                      <select
                        className="portal-input w-full text-[12px] font-bold bg-white"
                        value={rangerChoice}
                        onChange={(e) => setRangerChoice(e.target.value)}
                      >
                      ...
                      <div className="flex gap-2">
                        <button
                          className="portal-btn flex-1 text-[11px] h-9 shadow-sm"
                          disabled={!rangerChoice}
                          onClick={() => assign(i.incident_id)}
                        >
                          Deploy
                        </button>
                        <button
                          className="portal-btn portal-btn-ghost flex-1 text-[11px] h-9"
                    ...
                    <button
                      className="portal-btn portal-btn-ghost text-[11px] w-full justify-center mt-4 h-9 font-bold border-neutral-200"
                      onClick={() => setAssigning(i.incident_id)}
                    >
                      <UserCheck size={14} /> Assign Operator
                    </button>
                  )}
                </div>
              ))}
        ...
        <div className="col-span-4 space-y-4">
          <h3 className="portal-display text-sm font-black text-neutral-900 px-2 uppercase tracking-widest">
            Active Roster
          </h3>
          <div className="portal-card bg-white shadow-sm border-neutral-100 h-[700px] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-auto p-3 space-y-2 custom-scrollbar">
              {loadingRangers && (
                <div className="text-[12px] text-neutral-400 p-2 italic animate-pulse">
                  Authenticating personnel IDs…
                </div>
              )}
              {(rangers ?? []).map((r) => (
                <div
                  key={r.user_id}
                  className="p-4 rounded-2xl border border-neutral-50 bg-white flex items-center gap-4 hover:border-neutral-200 transition-colors"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-black text-neutral-800 leading-tight tracking-tight">
                      {r.first_name} {r.last_name}
                    </div>
                    <div className="mt-1 flex items-center gap-2">
                      <div className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
                      <span className="text-[10px] font-black text-neutral-400 uppercase tracking-widest">
                        {r.active_assignments_count} Operations
                      </span>
                    </div>
                  </div>
                </div>
              ))}
              {!loadingRangers && (rangers ?? []).length === 0 && (
                <div className="text-[12px] text-neutral-300 p-6 text-center italic">
                  Sector roster empty.
                </div>
              )}
            </div>
          ...
          <div className="portal-card bg-white shadow-sm border-neutral-100 h-[700px] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-auto p-3 space-y-3 custom-scrollbar">
              {inProgress.map((i) => (
                <div
                  key={i.incident_id}
                  className="p-4 rounded-2xl border border-neutral-100 bg-[#1A2F1A]/[0.02] shadow-sm"
                >
                  <div className="flex items-center justify-between text-[11px]">
                    <div className="font-mono font-black text-neutral-300 uppercase">
                      OPER WW-{i.incident_id}
                    </div>
                    <StatusBadge status={i.status} />
                  </div>
                  <div className="text-[13px] font-black text-neutral-800 mt-2">
                  ...
                  <div className="mt-4 flex items-center gap-3 p-3 bg-white rounded-xl border border-neutral-100 shadow-sm">
                    <div className="h-7 w-7 rounded-lg bg-[#1A2F1A] text-white flex items-center justify-center text-[10px] font-black shadow-lg">
                      {i.assignments?.[0]?.ranger?.first_name[0]}
                      {i.assignments?.[0]?.ranger?.last_name[0]}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-[11px] font-black text-neutral-800 truncate uppercase tracking-tight">
                        {i.assignments?.[0]?.ranger
                          ? `${i.assignments[0].ranger!.first_name} ${i.assignments[0].ranger!.last_name}`
                          : "Unassigned"}
                      </div>
                  ...
                  <div className="mt-3 text-[10px] font-bold text-neutral-300 uppercase tracking-widest">
                    Logged {fmtDate(i.created_at)}
                  </div>
                </div>
              ))}
              {inProgress.length === 0 && (
                <div className="text-[12px] text-neutral-300 p-6 text-center italic">
                  No active missions in field.
                </div>
              )}
            </div>
          </div>
      ...
      {err && (
        <div className="mt-6 text-[12px] font-black text-red-600 bg-red-50 border border-red-100 rounded-2xl px-4 py-3 animate-in shake-in duration-300">
          {err}
        </div>
      )}
    </PortalShell>
