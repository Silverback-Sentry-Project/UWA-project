...
`../.github/workflows/frontend-deploy.yml` (added 2026-08-13) runs on every push to `cleanup/aug-2026` or `main` touching this directory: `npm run typecheck` → `npm run build` → `wrangler deploy` to Cloudflare Workers as `wildwatch-portal`, deploy only on a clean build. Not `nitro deploy --prebuilt` (the command the build output itself suggests) — that has no way to pin the Worker name, and Nitro auto-derives one from build context otherwise (the original deploy used the auto-generated `silverback-sentry-project-uwa-project-frontend`, renamed 2026-08-13). Needs `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` repo secrets (set 2026-08-13, confirmed to have actually deployed). The `VITE_API_URL` build-time requirement from the manual procedure above is satisfied automatically for CI too — it's committed in `.env.production`, which Vite loads by default for any production build (`npm run build` = `vite build`, mode `production`), so the workflow doesn't need to set it explicitly.
